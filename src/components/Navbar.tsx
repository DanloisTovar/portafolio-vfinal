  ...
  return (
    <>
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-fit">
        <div
          className={twMerge(
            'flex items-center justify-between gap-1 sm:gap-2 p-1.5 sm:p-2 rounded-full border border-white/20 shadow-lg transition-all duration-300',
        ...
        >
          <a
            ...
            className="flex items-center gap-1 sm:gap-2 pl-2 hover:opacity-80 transition-opacity whitespace-nowrap"
          >
            <div className="p-1.5 rounded-full bg-blue-600/10 dark:bg-blue-400/10 text-blue-600 dark:text-blue-400">
              <Code size={25} />
            </div>
            <span className="font-bold text-xs sm:text-base font-orbitron">Danlois Tovar </span>
          </a>

          <div className="w-px h-6 bg-black/10 dark:bg-white/10 mx-0.5 sm:mx-1"></div>

          <div className="flex items-center gap-0.5 sm:gap-2">
            <button
              onClick={() => setIsOpen(true)}
              className="p-2 sm:p-3 rounded-full bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 transition-colors"
